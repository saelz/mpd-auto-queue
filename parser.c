...
enum PARSE_STATE { NONE, NAME, ATTR, DATA, SUB_ITEM };
...
append_xml_child(struct xml_item *item, struct xml_item *child) {
	item->children = realloc(item->children, (item->child_count + 1) *
	                                             sizeof(*item->children));
...
parse_xml(struct str xml_data) {
	...
	if (!find_next(xml_data, header, &pos)) {
		log_data(LOG_ERROR,
		         "Unable to find XML declaration in provided data");
		return NULL;
	}
...
find_xml_item_child(struct xml_item *item, struct str name) {
	...
	for (i = 0; i < item->child_count; i++) {
		...
		if (!strncmp(child->name.data, name.data,
		             (name.size > child->name.size ? child->name.size
		                                           : name.size)))
			return child;
	}
	return NULL;
...
print_xml_item(struct xml_item *item, int depth) {
	size_t i;
	printf("%*c$%.*s^ -> $%.*s^\n", depth, ' ', (int)item->name.size,
	       item->name.data, (int)item->data.size, item->data.data);
