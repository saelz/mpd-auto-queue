...
main(int argc, char *argv[]) {
	...
	if ((conn = mpd_connection_new(conf->mpd_host, conf->mpd_port, 0)) ==
	    NULL) {
	...
	if (conf->mpd_password != NULL &&
	    !mpd_run_password(conn, conf->mpd_password)) {
	...
	for (;;) {
		...
		while ((message = mpd_recv_message(conn)) != NULL) {
			...
			log_data(LOG_VERBOSE, "Received Message:%s",
			         message_text);
			if (!strcmp(message_text, "exit")) {
				...
				paused = 1;
			} else if (!strcmp(message_text, "resume") ||
			           !strcmp(message_text, "unpause")) {
				log_data(LOG_INFO, "Auto queuing has resumed");
				paused = 0;
			} else if (!strcmp(message_text, "toggle-pause")) {
				paused = !paused;
				log_data(LOG_INFO, "Auto queuing has %s",
		...
		if (!paused) {
			...
			if (status == NULL) {
				log_data(
				    LOG_ERROR,
				    "Unable to retrive MPD status: %s",
				    mpd_connection_get_error_message(conn));
				break;
			}
			...
			if (queue_len > 0 &&
			    queue_len - song_pos <= conf->min_songs_left)
				autoqueue(conn);
		...
		if (paused)
			...
			mpd_run_idle_mask(conn, MPD_IDLE_QUEUE |
			                            MPD_IDLE_PLAYER |
			                            MPD_IDLE_MESSAGE);
